...
public:
  CEpollListenThread() :
      m_pNetThreadRes(NULL), m_pNetEpoll(NULL)
  {
  }
  CEpollListenThread& operator=(const CEpollListenThread& cEpollListenThread)
  {
    m_pNetThreadRes = cEpollListenThread.m_pNetThreadRes;
    m_pNetEpoll = cEpollListenThread.m_pNetEpoll;
    return *this;
  }
  CEpollListenThread(const CEpollListenThread& cEpollListenThread) :
      m_pNetThreadRes(cEpollListenThread.m_pNetThreadRes), m_pNetEpoll(
          cEpollListenThread.m_pNetEpoll)
  {
  }
  ...
  void main()
  {
    m_pNetThreadRes->m_iThreadID = pthread_self();
    pthread_detach(m_pNetThreadRes->m_iThreadID);
    m_pNetEpoll->runListenBase(*m_pNetThreadRes);
  }
  int askToStop()
  {
	  return RETOK;
...
public:
  ...
  CEpollRunThread(const CEpollRunThread & cEpollRunThread) :
      m_pNetThreadRes(cEpollRunThread.m_pNetThreadRes), m_pNetEpoll(
          cEpollRunThread.m_pNetEpoll)
  {
  }
  CEpollRunThread& operator=(const CEpollRunThread& cEpollRunThread)
  {
    m_pNetThreadRes = cEpollRunThread.m_pNetThreadRes;
    m_pNetEpoll = cEpollRunThread.m_pNetEpoll;
    return *this;
  }
  int32_t init(SNetThreadRes & sNetThreadRes, CNetEpoll& impl)
  {
  ...
  void main()
  {
    m_pNetThreadRes->m_iThreadID = pthread_self();
    pthread_detach(m_pNetThreadRes->m_iThreadID);
    m_pNetEpoll->runDealSockBase(*m_pNetThreadRes);
  }
  int32_t askToStop()
  {
	  return m_pNetEpoll->askToStop();
  }
private:
  SNetThreadRes* m_pNetThreadRes;
  CNetEpoll* m_pNetEpoll;
...
int32_t CNetEpollImp::init(const STString& STStrConfigFile)
{
  ...
  for (uint32_t j = 1; j <= iNumThread; j++) {
    //log iNumThread
    m_runEvent.insert(std::make_pair<uint32_t, SNetThreadRes>(j, sNetThreadRes));
  }
...
int32_t CNetEpollImp::destroy()
{
  m_cEpollCfg.destroy();
  askToStop();
  std::vector<CEpollListenThread>::iterator itBeginLis = m_vctListenThread.begin();
  std::vector<CEpollListenThread>::iterator itEndLis = m_vctListenThread.end();
  while (itBeginLis != itEndLis) {
    itBeginLis->askToStop();
    while (itBeginLis->isRunning()) {
      sleep(1);
      continue;
    }
    ++itBeginLis;
...
int32_t CNetEpollImp::runListen(CNetEpoll& cNetEpoll)
{
  ...
  std::vector<CEpollListenThread>::iterator itListen =  m_vctListenThread.begin();
...
int32_t CNetEpollImp::runDealSockBase(SNetThreadRes& sNetThreadRes)
{
  ...
  std::map<SNet, STNetEvent>::iterator itActBegin;
  std::map<SNet, STNetEvent>::iterator itActEnd;
  while (1) {
    //log runDealSockBase;
    ...
    if (sNetThreadRes.sMapEventBef.size() > 0) {
      if (sNetThreadRes.m_mutexBef->tryLock()) {
        ...
        while (itBegin != itend) {
          ...
          itRet = sNetThreadRes.sMapEvent.insert(std::pair<SNet, SEvent>(itBegin->first, itBegin->second));
          int32_t iRet = 0;
          if (itRet.second) {
            sflag = 0;
            setsockopt(itRet.first->second.data.fd, SOL_SOCKET, SO_KEEPALIVE, &sflag, sizeof(sflag));
            setsockopt(itRet.first->second.data.fd, IPPROTO_TCP, TCP_NODELAY, &sflag, sizeof(sflag));
            itRet.first->second.events = EPOLLOUT | EPOLLIN | EPOLLET;
            iRet = epoll_ctl(sNetThreadRes.m_eFd, EPOLL_CTL_ADD, itRet.first->second.data.fd, &(itRet.first->second));
          }
          else {
...
//					itBegin = sNetThreadRes.sMapEventBef.begin();
        ...
        itActBegin = sNetThreadRes.sMapActBef.begin();
        itActEnd = sNetThreadRes.sMapActBef.end();
        while (itActBegin != itActEnd) {
        	sNetThreadRes.sMapAct.erase(itActBegin->first);
			sNetThreadRes.sMapAct.insert(std::make_pair<SNet, STNetEvent>(itActBegin->first,itActBegin->second));
			++itActBegin;
        }

        sNetThreadRes.sMapActBef.clear();
    ...
    iRet = epoll_wait(sNetThreadRes.m_eFd, sNetThreadRes.m_happenEvent,MAXHAPPENEVENT, sNetThreadRes.m_iTimeOut);
    //log
    if (iRet < 0) {
      //log
      ++iErrorCount;
    ...
    for (it = 0; it < iRet; it++) {
      ...
      if ((sNetThreadRes.m_happenEvent[it].data.fd < 0)
        ...
        //log
        epoll_ctl(sNetThreadRes.m_eFd, EPOLL_CTL_DEL,sNetThreadRes.m_happenEvent[it].data.fd, &(sNetThreadRes.m_happenEvent[it]));
        sNetThreadRes.m_mutex->lock();
        std::map<int32_t, SNet>::iterator itS2N = sNetThreadRes.sMapS2Net.find(sNetThreadRes.m_happenEvent[it].data.fd);
        if (itS2N != sNetThreadRes.sMapS2Net.end()) {
        	sNetThreadRes.sMapAct.erase(itS2N->second);
      ...
      if ((sNetThreadRes.m_happenEvent[it].events & EPOLLOUT)
        ...
        {
        	std::map<SNet, STNetEvent>::iterator itNEvent;
        	std::map<int32_t, SNet>::iterator itS2N = sNetThreadRes.sMapS2Net.find(sNetThreadRes.m_happenEvent[it].data.fd);
        	if (itS2N != sNetThreadRes.sMapS2Net.end())
        	{
        		itNEvent = sNetThreadRes.sMapAct.find(itS2N->second);
        		if (itNEvent != sNetThreadRes.sMapAct.end()) {
        	...
        	if (sNetThreadRes.sNetEvent.wcb != NULL){
        		 sNetThreadRes.sNetEvent.wcb(sNetThreadRes.m_happenEvent[it].data.fd,&(itNEvent->second),&iRet);
        	}
        }
        if (0 > iRet) {
          epoll_ctl(sNetThreadRes.m_eFd, EPOLL_CTL_DEL, sNetThreadRes.m_happenEvent[it].data.fd, &sNetThreadRes.m_happenEvent[it]);
          ...
          std::map<int32_t, SNet>::iterator itS2N =
              sNetThreadRes.sMapS2Net.find( sNetThreadRes.m_happenEvent[it].data.fd);
          if (itS2N != sNetThreadRes.sMapS2Net.end()) {
        	  sNetThreadRes.sMapAct.erase(itS2N->second);
            sNetThreadRes.sMapEvent.erase(itS2N->second);
            sNetThreadRes.sMapS2Net.erase(itS2N);
      ...
      if ((sNetThreadRes.m_happenEvent[it].events & EPOLLIN)
        ...
        //log sNetThreadRes.sNetEvent.wcb();
        {
        	...
        	if (sNetThreadRes.sNetEvent.rcb != NULL) {
        		sNetThreadRes.sNetEvent.rcb(sNetThreadRes.m_happenEvent[it].data.fd,&(itNEvent->second),&iRet);
        	}
        }
        if (0 > iRet) {
          epoll_ctl(sNetThreadRes.m_eFd, EPOLL_CTL_DEL,sNetThreadRes.m_happenEvent[it].data.fd,&sNetThreadRes.m_happenEvent[it]);
...
int32_t CNetEpollImp::runListenBase(SNetThreadRes & sNetThreadRes)
{
  ...
  while (1) {
    ...
    iRet = epoll_wait(sNetThreadRes.m_eFd, sNetThreadRes.m_happenEvent,MAXHAPPENEVENT, -1);
    //log iRet
    if (iRet < 0) {
      ++iErrorCount;
      //log
    ...
    for (it = 0; it < iRet; it++) {
      ...
      if (sNetThreadRes.m_happenEvent[it].events & EPOLLIN) //have connection request
        ...
        do {
          ...
          SEvent sEvent;
          SNet sNet;
          iSFd = accept(sNetThreadRes.m_happenEvent[it].data.fd,(struct sockaddr *) &clientaddr, &uiAddrSize);
          if (iSFd > 0) {
...
//thread to size
  ...
  std::map<uint32_t, SNetThreadRes>::iterator itEven = m_runEvent.find(itTmp->second.uThreadNum);
  if (itEven == m_runEvent.end()) {
    //log m_runEvent find RETERROR
    return RETERROR;
  }
  itEven->second.m_mutexBef->lock();
  std::map<int32_t, SNet>::iterator itS2N = itEven->second.sMapS2NetBef.find(sEvent.data.fd);
  if (itS2N != itEven->second.sMapS2NetBef.end()) {
	itEven->second.sMapActBef.erase(itS2N->second);
    itEven->second.sMapEventBef.erase(itS2N->second);
    itEven->second.sMapS2NetBef.erase(sEvent.data.fd);
  }
  STNetEvent sTNetEvent;
  itEven->second.sMapActBef.insert(std::pair<SNet, STNetEvent>(sNet, sTNetEvent));
  itEven->second.sMapS2NetBef.insert(std::pair<int32_t, SNet>(sEvent.data.fd, sNet));
...
int32_t CNetEpollImp::initBase(SNetThreadRes & sNetThreadRes)
{
  ...
  while (itBegin != sNetThreadRes.sMapEvent.end()) {
    ...
    int32_t iRet = epoll_ctl(sNetThreadRes.m_eFd, EPOLL_CTL_ADD,itBegin->second.data.fd, &(itBegin->second));
    if (0 > iRet) {
...
int32_t CNetEpoll::askToStop()
{
	return m_cNetEpollImp->askToStop();
}
